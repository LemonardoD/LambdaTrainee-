...
let finalJson =[]
for (el of daysOff){
  weekend.push(`{"name":"${el.user.name}","weekendDays":${`{"startDate":"${el.startDate}","endDate":"${el.endDate}"}`}}`)
  newDaysOff.push(`{"userId":"${el.user._id}","name":"${el.user.name}","weekendDays":[]}`)
}
newDaysOff = [...new Set(newDaysOff)]
for (let i = 0; i < newDaysOff.length; i++  ){
  let typo = JSON.parse(newDaysOff[i])
  daysOffJson.push(typo)
}
for (let i = 0; i < weekend.length; i++  ){
  let typo = JSON.parse(weekend[i])
  weekendJson.push(typo)
}
for (let a = 0; a < daysOffJson.length; a++  ){
  dates=[]
  for (let b = 0; b < weekendJson.length; b++  ){
    if(daysOffJson[a].name === weekendJson[b].name){
      dates.push(`${`{"startDate":"${weekendJson[b].weekendDays.startDate}","endDate":"${weekendJson[b].weekendDays.endDate}"}`}`)
    }
    
  } 
  final.push(`{"userId":"${daysOffJson[a].userId}","name":"${daysOffJson[a].name}","weekendDays":[${dates}]}`)
} 
for (let i = 0; i < final.length; i++  ){
  let typo = JSON.parse(final[i])
  finalJson.push(typo)
}
